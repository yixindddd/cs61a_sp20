...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        """
        ### Problem 3 solution
        def follow_entrance(place):
            if place.bees:
                return random_or_none(place.bees)
            if place.entrance:
                if place.entrance == beehive:
                    return None
                return follow_entrance(place.entrance)     
        return follow_entrance(self.place)
        """
        distance = 0
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        # if there are any bees in its place, attack them
        if self.place.bees:
            bees = list(self.place.bees)
            for bee in bees:
                bee.reduce_armor(amount)
        # remove the FireAnt if it has no armor remaining
        # apply the bonus if the fire ant dies
        if self.armor <= 0:
            if self.place.bees:
                for bee in bees:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        # Eat a random Bee in its place by reducing the Bee's armor to 0 and restart the digesting timer.
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        # check if it is digesting; if so, decrement its digesting counter.
        "*** YOUR CODE HERE ***"
        if self.digesting == 0:
            if self.place.bees:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        # reduce the armor of all Bees in the same place as the NinjaAnt by its damage attribute.
        if self.place.bees:
            bees = list(self.place.bees)
            for bee in bees:
                bee.reduce_armor(self.damage)
...
def apply_effect(effect, bee, duration):
    ...
    curr_action = effect(bee.action, bee)
    def new_action(gamestate):
        nonlocal duration
        if duration > 0:
            curr_action(gamestate)
            duration -= 1
        else:
            prev_action(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        def forward(place):
            nonlocal distance
            if place.entrance == beehive:
                insect_list.pop(self)
                return insect_list
            if place.ant:
                insect_list[place.ant] = distance
            if place.bees:
                for bee in place.bees:
                    insect_list[bee] = distance
            distance += 1
            return forward(place.entrance)
        return forward(self.place)
        # return {}
        # END Problem OPTIONAL
    ...
    def calculate_damage(self, distance):
        ...
        if self.base_damage > 0:
            self.base_damage = LaserAnt.base_damage - 0.05 * self.insects_shot
            return self.base_damage - 0.2 * distance
        # return 0
        # END Problem OPTIONAL
