...
def closer_city(lat, lon, city1, city2):
    ...
    distance1 = distance(destination, city1)
    distance2 = distance(destination, city2)
...
def nut_finder(t):
    ...
    if label(t) == 'nut':
        ...
        nut_counts = [nut_finder(b) for b in branches(t)]   
    return True if True in nut_counts else False
...
def add_trees(t1, t2):
    ...
    if len(b1) < len(b2):
        ...
        b2 += [''] * (len(b1) - len(b2))
    return tree(label(t1) + label(t2), [add_trees(i1, i2) for i1, i2 in zip(b1, b2)])
