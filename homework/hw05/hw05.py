...
def preorder(t):
    ...
    entry_list = []
    entry_list.append(t.label)
    for branch in t.branches:
        if branch.is_leaf():
            entry_list.append(branch.label)
        else:
            entry_list += preorder(branch)
    return entry_list
...
def generate_paths(t, value):
    ...
    "*** YOUR CODE HERE ***"
    """
    # My stupid solution: track every path, and only keep ones that contain the value
    paths = []
    def track(t):      
        paths.append(t.label)
        if t.is_leaf():
            if value in paths:
                index = [i for i,v in enumerate(paths) if v == value]
                for i in index:
                    yield paths[:(i+1)] 
        else:
            for branch in t.branches:
                for t in track(branch):
                    yield t
                paths.pop()
    result = []
    for path in track(t):
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if bst_max(left_branch) > t.label:
            return False
        if bst_min(right_branch) <= t.label:
            return False
