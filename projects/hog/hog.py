...
def is_swap(player_score, opponent_score):
    ...
    return int(str(sum)[0]) == int(str(sum)[-1])
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal) and (score1 < goal):
        ...
        if who == 0:
            ...
            if feral_hogs and (abs(thisTurn0 - num_rolls) == 2):
                  score0 += 3
            thisTurn0 = take_turn(num_rolls, score1, dice)
            score0 += thisTurn0
        else:
            num_rolls = strategy1(score1, score0)
            if feral_hogs and (abs(thisTurn1 - num_rolls) == 2):
                  score1 += 3
            thisTurn1 = take_turn(num_rolls, score0, dice)
            score1 += thisTurn1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score = make_averaged(roll_dice, num_samples)
    result, high_result, i = 0, 0, 1
    while i <= 10:
        result = max_score(i, dice)
        if result > high_result:
            high_result = result
            dice_num = i
        i = i + 1
    return dice_num
